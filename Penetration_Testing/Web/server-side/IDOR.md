...
```bash
### We have a link that reveals resources:
...
### getting only the documents links
...
A bash script to loop over the UID parameter
...
Running the script will download all documents from all employees with `uids` between 1-10, exploiting the IDOR vulnerability to mass enumerate the documents of all employees.
