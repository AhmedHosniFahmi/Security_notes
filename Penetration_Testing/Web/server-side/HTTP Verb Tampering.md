...
Request before HTTP verb tampering
...
``` HTML
</ul>Malicious Request Denied!</ul>
```

Request after HTTP verb tampering
...
Vulnerable configurations located in the site configuration file (e.g. `000-default.conf`), or in a `.htaccess`.
...
Vulnerable configurations located in the `web.xml` file for a certain Java web application.
...
Vulnerable configurations located in the `web.config` file of a web application.
